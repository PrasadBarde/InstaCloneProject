...
mongoose.connect(process.env.DATABASE_URL || "mongodb+srv://InstaClone10x:zxp38MRj2ZKhLy4P@cluster1.38egeje.mongodb.net/Instaclone?retryWrites=true&w=majority").then(() => {
    postimage.find({}).then((data) => {
        ...
        app.listen(process.env.PORT || 8001, (err) => {
            if (err) {
                ...
                console.log("Server is Running sucessfully 8001")
            }
        })
    })
})
...
app.get("/", (req, res) => {
    res.send("InstaClone Backend By Prasad Barde")
})
